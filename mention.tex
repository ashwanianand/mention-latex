...
need the most predictable behavior should use a local \file{phfcc-local.cfg}
file.
...
package chooses the next color from its built-in cycle.
...
\begin{enumerate}
  ...
  \item a local \file{phfcc-local.cfg} file, if present;
...
\file{phfcc-local.cfg} ignored and let each collaborator create their own copy
from \file{phfcc-local.cfg.example}.
...
The package loads \file{phfcc-local.cfg} automatically when the document
starts.
...
viewer selection through \file{phfcc-local.cfg} does not require shell escape.
...
\file{mention.sty}, \file{phfcc-local.cfg.example}, and
