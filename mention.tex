...
When \opt{color} is omitted, \pkg{mention} assigns an author color
automatically.  If the \pkg{colorblind} package is available, the default cycle
uses private copies of Paul Tol's muted |T-Q-M1| through |T-Q-M9| colors.
\pkg{mention} loads \pkg{colorblind} with \opt{Tol,keep-defaults,no-tikz}, so
default document colors such as |red|, |green|, or |blue| are not changed.

If \pkg{colorblind} is missing, or was already loaded without defining the
|T-Q-M| colors, \pkg{mention} falls back to its built-in \pkg{xcolor} palette.
Explicit author colors always take precedence:
...
The special recipient-list item \opt{all} routes a comment to every active
viewer.  Every viewer who compiles the document sees the margin note and gets
the comment included in their unresolved-count total.
...
note, and does not contribute to unresolved counts.  If \opt{all} and
\opt{resolved} are both present, \opt{resolved} wins.
...
\file{targeted-comments-demo.tex}, and the derived files \file{mention.pdf}
and \file{targeted-comments-demo.pdf}.
